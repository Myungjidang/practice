...
def clustering(n_clusters):
    ...
    path = 'C:\\Users\\MJ\\Desktop\\오목눈이\\save_data\\'
    country_list = ['korean', 'english', 'german', 'chinese', 'japanese']
    save_path = 'C:\\Users\\MJ\\Desktop\\오목눈이\\save_data\\'
    concat_doc_vector = torch.tensor([])
    word_v = torch.tensor([])
    word_list = np.array([])
    for country in country_list:
        try:
            country_data = pd.read_csv(path + f'tokenized_{country}_test_data.csv', encoding='utf-8-sig')
            globals()[country + '_document'] = make_document(country_data)
            globals()[country + '_data'] = pd.read_csv(path + f'tokenized_{country}_test_data.csv',
                                                       encoding='utf-8-sig')  # 없으면 밑에 실행 안되게끔
            globals()[country + '_word_list'] = np.load(path + f'top_word_list_{country}_test.npy')
            globals()[country + '_doc_v'] = torch.tensor(np.load(path + f'prj_doc_{country}_test.npy'))
            globals()[country + '_word_v'] = torch.tensor(np.load(path + f'prj_word_{country}_test.npy'))
    ...
    fig.show()
    path = 'C:\\Users\\MJ\\Desktop\\오목눈이\\save_data\\'
    fig.write_html(path + 'treemap.html')

    # Excel
    print('-------Final_table 생성중-------')
    writer = pd.ExcelWriter(path + 'Topic_check_multi.xlsx', engine='xlsxwriter')
